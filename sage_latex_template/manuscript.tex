...
\RMchange{An important aspect of DSLs is to be able to use domain knowledge for their compiler transformations, thus enabling code optimizations that may not be detected by general purpose compilers.
...
\RMchange{The P4IRS IR can be constructed in two ways: (a) by mapping of the Python IR generated by triggering the \emph{compile()} method and (b) by explicitly constructing the IR elements in-place.
