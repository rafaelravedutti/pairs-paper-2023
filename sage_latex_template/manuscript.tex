...
\autoref{fig:pairs_arch} depicts a general overview of the P4IRS architecture from the user inputs until the final binary.
...
\RMchange{P4IRS creates ``modules'' for one or more routines when building its IR}, and every module determines a point where data must be synchronized for the target it will execute on.
...
Although this is not a fair comparison as we use a single CPU core, it demonstrates that we can achieve a significant performance improvement when running P4IRS code on a GPU, and the results are close (and better in some cases) when comparing P4IRS code to MD-Bench for both CPU and GPU.}
...
Better performance is obtained when both the Neighbor-Lists per Cell and the Multi-Kernel approach are used for GPUs, but there are still room for improvement.
