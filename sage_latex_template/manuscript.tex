...
Most of P4IRS internal routines such as building Linked Cells and Verlet Lists are implemented in Python by constructing the intermediate representation for them with the assistance of operator overloading (see \nameref{sec:backend} section).
...
In the \nameref{sec:evaluation} section we evaluate the performance and scalability from P4IRS applications using \ac{MD} and \ac{DEM} experiments.
Finally, the \nameref{sec:conclusion} section presents our conclusion and outlook for P4IRS, also covering our future work.
...
Note that we use the term \ac{IR} in this paper to denominate the internal data structure utilized to represent the parsed code.
Our \ac{IR} is not formally defined as is usually the case for most \ac{IR}s from general purpose compilers, and it is not our intention here to provide a formal and strong definition for it.
Instead, we present an informal and rather simple \ac{AST} representation which contains elements associated with particle computations and data structures.
...
\begin{figure*}[htb]
  ...
  \caption{\RMchange{P4IRS IR for the BuildNeighborLists example. Red boxes represent elements that are specific to particle simulations, and are used for domain-specific transformations in the \ac{AST}, at some point these elements are lowered to generic, primitive elements that should be straightforward to map into code in a general-purpose language or even an IR such as the LLVM IR. Green boxes represent some of these lower level, generic elements. Blue boxes represent symbols from the simulation, and the dashed lines in red show when these symbols come from a particle-specific element. Yellow boxes represent constants, and both ``s'' elements represent a pattern that is repeated in the constructed \ac{AST} according to the referenced loop in their respective orange boxes.}}
  \label{fig:pairs_ir}
\end{figure*}
...
The latter is particulary important for the shape-partitioning and multi-kernel code generation discussed in the \nameref{sec:shape_partitioning} section.
...
Although most of the code generation is straightforward for the \ac{IR} in its lowest level, there are important aspects that must be considered to handle different backends and achieve performance-portability.
