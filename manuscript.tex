...
%    \hline
    Micro-architecture & Cascade Lake & Ice Lake server & Zen3 \\
    \hline
    Model & Xeon Gold 6248 & Xeon Platinum 8360Y & EPYC 7543 \\
    Base frequency & 2.4\,\GHZ & 2.5\,\GHZ & 2.1\,\GHZ \\
    Cores per chip & 20 & 36 & 32 \\
    Latest SIMD extension& AVX512 & AVX512 & AVX2+FMA \\
    L1D capacity & $20 \times 32$\,\KiB & $36 \times 48$\,\KiB & $32 \times 32$\,\KiB \\
    L2 capacity & $20 \times 1024$\,\KiB & $36 \times 1.25$\,\MiB & $32 \times 512$\,\KiB \\
    L3 capacity & 27.5\,\MiB & 54\,\MiB & 256\,\MiB \\
    %L1/L2 bandwidth &
    %L2/L3 bandwidth &
    Memory configuration & 6 ch. DDR4-2933 & 8 ch. DDR4-3200 & 8 ch. DDR4-3200 \\
%    \hline
  \end{tabular}
  \caption{Specifications for testbed CPUs.}
  \label{tab:testbedcpus}
\end{table*}
...
For single-core CPU experiments, we make use of the same clusters, where the two CPUs listed above will be referred to as Ice Lake and Milan, respectively.
...
The \ac{DEM} test case is a "Settling Spheres" benchmark, where spheres fall into the ground (half-space) to generate a "bed" of spherical particles, and due to its multiple shapes and dynamics with many interactions among spheres and half-spaces provide a good reference for DEM simulations.
We compare our single-core results with MESA-PD using 2 MPI ranks (since it is not possible to run in a single rank with PBC in MESA-PD), GPU results are not available for MESA-PD as it is one of its limitations, but we show the performance gain when using GPUs for P4IRS generated codes.

Scaling results are also shown for demostrating P4IRS scaling capabilities, and regular domnain-partitioning is used as these experiments do not require sophisticated load-balancing algorithms (Copper lattice is an homogeneous testcases in all dimensions, and Settling Spheres can obtain good weak-scaling by regularly partitioning the domain in X and Y dimensions).
We use double floating-point precision and display results when enabling and disabling Newton 3rd Law optimization (half neighbor-lists), for DEM we show both cases with Linked Cells and Verlet Lists.
...
\end{tikzpicture}
\vspace{-3ex}
	\caption{Execution time for P4IRS and MD-Bench simulations in seconds (lower is better) of a Copper FCC lattice system with $32^{3}$ unit-cells (4 atoms per unit-cell) during 200 time-steps on Ice Lake and Milan CPUs. Tests were performed in a single core (no parallelism) with fixed frequency. Results are shown for both full neighbor-lists (FN) and half neighbor-lists (HN) cases.}
\vspace{-2ex}
\centering
...
%        \hline
    \end{tabular}
    \caption{Measurements from P4IRS and MD-Bench Lennard-Jones force kernels for both full and half neighbor-lists variants on a single Ice Lake CPU core.}
...
\end{tikzpicture}
\vspace{-3ex}
	\caption{Execution time for P4IRS and MD-Bench simulations in seconds (lower is better) of a Copper FCC lattice system with $50^{3}$ unit-cells (4 atoms per unit-cell) during 200 time-steps on A40 and A100 GPUs. Results are shown for both full neighbor-lists (FN) and half neighbor-lists (HN) cases, and atomic operations affect the performance for the HN case.}
\vspace{-2ex}
\centering
...
\end{tikzpicture}
\vspace{-3ex}
\caption{Weak-scaling results for P4IRS (FN and HN) on Fritz, each node simulates 200 time-steps of a Copper FCC lattice system with $128^3$ unit cells, with approximately 116508 atoms per CPU core.}
\vspace{-2ex}
...
\end{tikzpicture}
\vspace{-3ex}
\caption{Weak-scaling results for P4IRS (FN and HN) on Alex on both A40 and A100 GPUs, each node simulates 200 time-steps of a Copper FCC lattice system with $100^3$ unit cells, with approximately 500000 atoms per GPU.}
\vspace{-2ex}
...
% DEM
%   - Comparison with MESA-PD (figure)
