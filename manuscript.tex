...
\begin{tikzpicture}[scale=0.95]
    ...
    \begin{axis}[bar shift=-24pt, nodes near coords style={xshift=-24pt},
        ...
        \addlegendimage{empty legend}
        \addlegendentry{\hspace{-.325cm}\textbf{B:} P4IRS+NBB}
        \addlegendimage{empty legend}
        \addlegendentry{\hspace{-.325cm}\textbf{C:} P4IRS+2R}
        \addlegendimage{empty legend}
        \addlegendentry{\hspace{-.325cm}\textbf{D:} P4IRS+NBB+2R}
        \addlegendimage{empty legend}
...
\end{tikzpicture}
\vspace{-3ex}
	\caption{Execution time for P4IRS and MD-Bench simulations in seconds (lower is better) of a Setting-Spheres benchmark with 18720 particles during 10000 time-steps on Ice Lake and Milan CPUs. Tests were performed in a single core (no parallelism) with fixed frequency. Results are shown for standard Linked Cells, Linked Cells with neighbor-lists created per bin (+NBB) and for single and two ranks (+2R).}
\vspace{-2ex}
\centering
...
For more complex kernels, there may be even more opportunities for specializations, which can result in even better performance improvements.
...
\begin{tikzpicture}[scale=0.95]
    ...
    \begin{axis}[bar shift=-18pt, nodes near coords style={xshift=-24pt},
        ...
        \addlegendimage{empty legend}
        \addlegendentry{\hspace{-.325cm}\textbf{D:} P4IRS+NBB+MK}
        %\addlegendimage{empty legend}
        %\addlegendentry{\hspace{-.325cm}\textbf{E:} MESA-PD+2R}
    \end{axis}
