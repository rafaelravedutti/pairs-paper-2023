...
Many performance-focused researches exist for MD systems, most of them using MD packages as a black-box and adjusting different simulation parameters to analyse their implications in the overall performance.
...
A collision detection step is required to check when two particles overlap, and parameters about the collision such as the contact point and penetration depth are calculated and used in the force calculation.
...
Considering $\hat{c}_{ij}$ the contact point and $\hat{n}_{ij}$ the normal direction vector between the particles $i$ and $j$, the relative contact velocity and their normal and tangential components can be expressed as:
...
Where $k^{n}$ is the spring stiffness, $\xi^{n}$ the normal velocity damping, $\mu$ the friction, and $\delta_{ij}$ the penetration depth.
...
From the algorithmic point of view, this means that a contact history data structure is required to keep track of the last updated value.
