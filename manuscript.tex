...
MESA-PD is a general particle dynamics framework that is built-in the waLBerla framework for the simulation of particles and fluids (with the Lattice-Boltzmann method).
...
One of tinyMD's limitation is that its flexibility and optimization settings are restricted to what can be achieve through higher-order functions and the AnyDSL compilation framework (the generated LLVM IR restricts the backend compilation to the Clang compiler), overcoming such limitation is one of the reason we develop P4IRS.
...
When computing forces for DEM simulations that contain different shapes, contact elements such as the penetration depth, contact point and even the distance are computed differently.
...
To provide a simpler way to achieve vectorized code for such DEM kernels, we partition particles by shape in our acceleration data structures (Linked Cells and Verlet Lists), which allows us to distinguish them during the force computation.
...
This is done by combining its runtime functions and code generation techniques, and allows us to make use of the waLBerla domain-partitioning scheme based on a block-forest data structure.
...
% Testbed
% Cascade Lake, Ice Lake, Milan, A40, A100
...
% Half vs Full neighbor-lists
% SP vs DP maybe
% Show FLOPS_DP/SP info for one architecture (Ice Lake) such as Vec. Ratio
